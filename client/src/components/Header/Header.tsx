...
import { userCredentials } from "config/mock/fakeData";
...
const MainNav: FC<MainNavProps> = () => {
  ...
  const [loggedText, setLggedText] = useState("Cargando la sesión...");
  ...
  useEffect(() => {
    const automaticLogin = async () => {
      ...
      setLggedText("Acceder a mi cuenta")
      setLoading(false);
    ...
    return (
      <div className="h-20 flex justify-between">
        ...
        <div className="flex-1 flex items-center justify-end text-slate-700 dark:text-slate-100">
          ...
          {isLogged ? (
            <>
              <AvatarDropdown onClose={handleLoggedIn}/>
              <CartDropdown />
            </>
          ) : (
            <>
